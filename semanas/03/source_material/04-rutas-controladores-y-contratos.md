# Rutas, controladores y el contrato
...
Los **path params** identifican un recurso puntual y son parte de la ruta. Cuando ves `/projects/{id}` y pedís `/projects/4`, ese `4` es un path param: dice "este proyecto, no otro". El path param identifica.
...
El **body** lleva el payload de una creación o actualización. Cuando hacés POST a `/projects/{id}/tasks`, el `id` viaja en la ruta porque identifica al proyecto, pero los datos de la tarea nueva (su título, su fecha de vencimiento) viajan en el body, normalmente como JSON. El body es el contenido de la operación.

La regla mental: path identifica, query modifica, body transporta el contenido nuevo.

## El controlador: quién atiende la promesa

Del lado del servidor, el **controlador** es la función que recibe el request y produce la response. Es quien atiende la promesa que la ruta hace. En este curso vos no vas a escribir controladores a mano: los vas a *especificar*, y la IA los va a implementar. La unidad mínima que le entregás a la IA no es código; es un bloque de contrato.
...
Ese bloque entero es la unidad mínima que se le dicta a la IA sin ambigüedad. Verbo, ruta, qué entra, qué sale en éxito, qué sale en cada falla anticipada. No queda decisión silenciosa para que el modelo improvise: si falta `title`, no es opinable, está escrito que devuelve 400.
